...
import { Edit, Plus, X, Users } from 'lucide-react';

const EditGroupModal = ({ group, onSave, onClose, isOpen, triggerButton }) => {
  const [formData, setFormData] = useState({
    name: group?.name || '',
    destination: group?.destination || '',
    start_date: group?.startDate || '',
    end_date: group?.endDate || '',
    status: group?.status || 'próximo'
  });

  const [pilgrims, setPilgrims] = useState(group?.pilgrims || []);
  const [newPilgrim, setNewPilgrim] = useState({ name: '', email: '' });
